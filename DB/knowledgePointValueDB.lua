...
local print = CraftSim.DEBUG:RegisterLogger("Database.knowledgePointValueDB")
...
function CraftSim.DB.KNOWLEDGE_POINT_VALUE:CleanUp()
    ...
    for crafterUID, nodeEntries in pairs(CraftSimDB.KnowledgePointValueDB.data) do
        for nodeID, entry in pairs(nodeEntries) do
            -- Skip weekly/respec plan sentinel keys (string keys) and entries without lastUpdated
            if type(nodeID) == "number" and entry.lastUpdated and (now - entry.lastUpdated) > maxAge then
...
function CraftSim.DB.KNOWLEDGE_POINT_VALUE:GetAllSorted(crafterUID)
    ...
    for key, entry in pairs(CraftSimDB.KnowledgePointValueDB.data[crafterUID]) do
        if type(key) == "number" and entry.currentRank and entry.maxRank and entry.currentRank < entry.maxRank then
...
--- Respec Plan persistence ---

---@class CraftSim.KnowledgePointValue.RespecPlan
---@field path CraftSim.KnowledgePointValue.PathStep[]
---@field availablePoints number total KP budget used when plan was created
---@field totalGain number cumulative ROI of the blank-slate path
---@field profession Enum.Profession
---@field savedAt number time() timestamp
---@field topNodeName string? name of the best first step
---@field comparison CraftSim.KnowledgePointValue.RespecComparison?

---@param crafterUID CrafterUID
---@param profession Enum.Profession
---@param plan CraftSim.KnowledgePointValue.RespecPlan
function CraftSim.DB.KNOWLEDGE_POINT_VALUE:SaveRespecPlan(crafterUID, profession, plan)
    CraftSimDB.KnowledgePointValueDB.data[crafterUID] = CraftSimDB.KnowledgePointValueDB.data[crafterUID] or {}
    CraftSimDB.KnowledgePointValueDB.data[crafterUID]["__respecPlan_" .. profession] = plan
end

---@param crafterUID CrafterUID
---@param profession Enum.Profession
---@return CraftSim.KnowledgePointValue.RespecPlan?
function CraftSim.DB.KNOWLEDGE_POINT_VALUE:GetRespecPlan(crafterUID, profession)
    if not CraftSimDB.KnowledgePointValueDB.data[crafterUID] then return nil end
    return CraftSimDB.KnowledgePointValueDB.data[crafterUID]["__respecPlan_" .. profession]
end
