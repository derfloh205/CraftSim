...
function CraftSim.FRAME:UpdateProfitDetails(recipeData, calculationData)
    ...
    local profitCalculationTextMC = "((1-BASE% * BASE_V)*(MC% * MC_V)*0.95) - (CCC - RES) = " .. CraftSim.UTIL:FormatMoney(calculationData.meanProfit, true)
    local profitCalculationTextINSP = "((1-BASE% * BASE_V)*(INSP%% * INSP_V)*0.95) - (CCC - RES)  = " .. CraftSim.UTIL:FormatMoney(calculationData.meanProfit, true)
    local profitCalculationTextMC_INSP = "((1-BASE% * BASE_V) * (MC% * MC_V) * (INSP% * INSP_V) * (MC_INSP% * MC_INSP_V) * 0.95) - (CCC - RES) = " .. CraftSim.UTIL:FormatMoney(calculationData.meanProfit, true)

    local profitCalculationText = ""
    -- TODO switch what calculation is shown depending of enabled stats

    profitCalculationText = profitCalculationText .. CraftSim.UTIL:FormatMoney(calculationData.meanProfit, true)
...
function CraftSim.FRAME:InitProfitDetailsFrame()
	...
	frame.content.description:SetText(
        "The Ø (Ø = Average) Profit is calculated by the weighted sum of all possible procc combinations\n\n"..
        CraftSim.UTIL:ColorizeText("((The Ø value of a craft result proccing nothing, weighted by its chance)\n", blue) ..
        CraftSim.UTIL:ColorizeText("*\n", green) ..
        CraftSim.UTIL:ColorizeText("(The Ø value of a craft result proccing multicraft, weighted by its chance)\n", blue) ..
        CraftSim.UTIL:ColorizeText("*\n", green) ..
        CraftSim.UTIL:ColorizeText("(The Ø value of a craft result proccing inspiration, weighted by its chance)\n", blue) ..
        CraftSim.UTIL:ColorizeText("*\n", green) ..
        CraftSim.UTIL:ColorizeText("(The Ø value of a craft result proccing both inspiration and multicraft, weighted by its chance)\n", blue) ..
        ...
        CraftSim.UTIL:ColorizeText("-\n", red) ..
        CraftSim.UTIL:ColorizeText("The Ø material cost saved from resourcefulness)\n\n", blue) ..
    ...
    frame.content.valueText = frame.content:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	frame.content.valueText:SetPoint("TOP", frame.content.averageProfitValue, "TOP", 0, -30)
	frame.content.valueText:SetText("...")
    frame.content.valueText:SetJustifyH("LEFT")
