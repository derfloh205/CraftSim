    ...
    local function createContent(frame)
        ...
        frame.content.knowledgePointSimulationButton = GGUI.Button({
            ...
            clickCallback=function ()
                ...
                CraftSim.FRAME:ToggleFrame(GGUI:GetFrame(CraftSim.MAIN.FRAMES, CraftSim.CONST.FRAMES.SPEC_SIM), not specSimFrame:IsVisible())
            end
        })

        frame.content.knowledgePointSimulationButton:SetEnabled(false)

        frame.content.notImplementedText = CraftSim.FRAME:CreateText(GUTIL:ColorizeText(CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.SPEC_INFO_WORK_IN_PROGRESS), GUTIL.COLORS.LEGENDARY),
    ...
    for _, affectedNodeData in pairs(affectedNodeDataList) do
        content.nodeList:Add(function (row)
            ...
            if affectedNodeData.active then
                ...
                rankColumn.text:SetText("(" .. tostring(affectedNodeData.rank) .. "/" .. tostring(affectedNodeData.maxRank) .. ")")
            else
                nameColumn.text:SetText(GUTIL:ColorizeText(affectedNodeData.nodeName, GUTIL.COLORS.GREY))
                rankColumn.text:SetText(GUTIL:ColorizeText("(-/" .. tostring(affectedNodeData.maxRank) .. ")", GUTIL.COLORS.GREY))
            end
