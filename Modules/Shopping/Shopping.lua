...
function CraftSim.SHOPPING.UI:Init()
    -- frame is created lazily on first open
end
...
function CraftSim.SHOPPING:AddSearchTermToShoppingList(searchTerm, listName)
    ...
    if api.AddToShoppingList then
        ...
        success, result = pcall(api.AddToShoppingList, listName, { searchTerm })
        if success and result ~= false then
            return true
        end
    end
...
function CraftSim.SHOPPING:AddRecipeToShoppingList(recipeID, recipeName, sourceText)
    ...
    if not self:IsAuctionatorAvailable() then
        CraftSim.DEBUG:SystemPrint(
            f.l("CraftSim: ") .. f.r(L("RECIPE_ACQUISITION_SHOPPING_LIST_AUCTIONATOR_REQUIRED")))
        return false
    end
    ...
    if not self:AddSearchTermToShoppingList(searchTerm) then
        CraftSim.DEBUG:SystemPrint(
            f.l("CraftSim: ")
                .. string.format(L("RECIPE_ACQUISITION_FAILED_TO_ADD_TO_SHOPPING_LIST"), f.bb(shoppingSearch.itemName)))
        return false
    end
...
function CraftSim.SHOPPING:CreateShoppingListViewFrameIfNeeded()
    ...
    local frame = GGUI.Frame({
        ...
        sizeX = 520,
        sizeY = 280,
        title = L("CONTROL_PANEL_MODULES_SHOPPING_LIST_LABEL"),
        collapseable = true,
        closeable = true,
        moveable = true,
