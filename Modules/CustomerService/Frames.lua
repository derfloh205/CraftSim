...
function CraftSim.CUSTOMER_SERVICE.FRAMES:Init()
    local frame = GGUI.Frame({
        ...
        onCloseCallback = CraftSim.CONTROL_PANEL:HandleModuleClose("MODULE_CUSTOMER_SERVICE"),
    ...
    local function createContent(frame)
        frame:Hide()
        local recipeWhisperTab = CraftSim.FRAME:CreateTab(
            CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_RECIPE_WHISPER), frame.content, frame.title
            .frame,
            "TOP", "BOTTOM", -90, -15, true, 300, 250, frame.content, frame.title.frame, 0, -50)
        local autoResultTab = CraftSim.FRAME:CreateTab(
            CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_LIVE_PREVIEW), frame.content, recipeWhisperTab,
            "LEFT", "RIGHT", 0, 0, true, 300, 250, frame.content, frame.title.frame, 0, -50)

        recipeWhisperTab.content.customerCharacterInput = CraftSim.FRAME:CreateInput(nil, recipeWhisperTab.content,
            recipeWhisperTab.content, "TOPLEFT", "TOPLEFT", 10, -30, 150, 25,
            "",
            function()
            end)
        ...
        recipeWhisperTab.content.msgFrameContent.msgFormatBox = CreateFrame("EditBox", nil,
        ...
        recipeWhisperTab.content.messageFormatTitle = CraftSim.FRAME:CreateText(
            CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_MESSAGE_FORMAT),
            recipeWhisperTab.content, recipeWhisperTab.content.msgFormatScrollFrame, "BOTTOMLEFT", "TOPLEFT", 8, 5)

        recipeWhisperTab.content.messageFormatHelp = CraftSim.FRAME:CreateHelpIcon(
...
function CraftSim.CUSTOMER_SERVICE.FRAMES:InitLivePreview()
    local frame = GGUI.Frame({
        ...
        sizeX = 500,
        sizeY = 500,
        frameID = CraftSim.CONST.FRAMES.LIVE_PREVIEW,
        frameStrata = "DIALOG",
        closeable = true,
        collapseable = true,
        moveable = true,
    ...
    local function createContent(frame)
        ...
        frame.content.previewTitle = CraftSim.FRAME:CreateText(
        ...
        frame.content.recipeDropdown = GGUI.Dropdown({
            ...
            offsetY = -30,
            sizeX = 200,
            clickCallback = onRecipeSelected,
        ...
        frame.content.loadingText = CraftSim.FRAME:CreateText(requestingUpdate, frame.content,
            frame.content.recipeDropdown.frame, "LEFT", "RIGHT", -20, 5, 0.8, nil, { type = "H", value = "LEFT" })
        ...
        local function checkForTimeOut(updateID)
            if tContains(frame.content.updates, updateID) then
                frame.content.loadingText:SetText(CraftSim.GUTIL:ColorizeText(
                    CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_TIMEOUT), CraftSim.GUTIL.COLORS.RED))
        ...
        frame.content.guaranteeCB = CraftSim.FRAME:CreateCheckboxCustomCallback(
            CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_HIGHEST_GUARANTEED_CHECKBOX),
        ...
        frame.content.optionalDropdownGroup:SetSize(dropdownWidth * 2 + dropdownOffsetX,
            -1 * dropdownBaseY + -1 * dropdownSpacingY * 2 + 25)
        ...
        frame.content.optionalDropdownGroup.SetCollapsed = function(collapsed)
            if collapsed then
                ...
                frame.content.optionalDropdownGroup:SetSize(dropdownWidth * 2 + dropdownOffsetX,
                    -1 * dropdownBaseY + -1 * dropdownSpacingY * 2 + 10)
        ...
        local function CreateReagentInputDropdown(label, offsetX, offsetY)
            local optionalReagentDropdown = GGUI.Dropdown({
                ...
                offsetY = offsetY,
                width = dropdownWidth,
                clickCallback = onOptionalReagentSelected,
            })
        ...
        frame.content.optionalDropdowns = {
            CreateReagentInputDropdown(
                CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_REAGENT_OPTIONAL) .. " 1", -dropdownOffsetX,
                dropdownBaseY),
            CreateReagentInputDropdown(
                CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_REAGENT_OPTIONAL) .. " 2", -dropdownOffsetX,
                dropdownBaseY + dropdownSpacingY),
            CreateReagentInputDropdown(
                CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_REAGENT_OPTIONAL) .. " 3", -dropdownOffsetX,
                dropdownBaseY + dropdownSpacingY * 2),

            CreateReagentInputDropdown(
                CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_REAGENT_FINISHING) .. " 1", dropdownOffsetX,
                dropdownBaseY),
            CreateReagentInputDropdown(
                CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_REAGENT_FINISHING) .. " 2", dropdownOffsetX,
                dropdownBaseY + dropdownSpacingY),
        }

        frame.content.craftingCosts = CraftSim.FRAME:CreateText(
            CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_CRAFTING_COSTS) ..
            "\n" .. CraftSim.GUTIL:FormatMoney(0),
            frame.content, frame.content.optionalDropdownGroup, "TOP", "BOTTOM", 0, -20)

        frame.content.expectedResultTitle = CraftSim.FRAME:CreateText(
        ...
        frame.content.expectedInspirationPercent = CraftSim.FRAME:CreateText(
        ...
        frame.content.reagentDetailsTitle = CraftSim.FRAME:CreateText(
            CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_REQUIRED_MATERIALS),
        ...
        local function createReagentFrame(anchorA, anchorParent, anchorB, anchorX, anchorY)
            ...
            reagentFrame.countTextNoQ = CraftSim.FRAME:CreateText(" x ???", reagentFrame, reagentFrame.icon.frame, "LEFT",
                "RIGHT", 5, 0, 0.9, nil, { type = "HV", valueH = "LEFT", valueV = "CENTER" })

            reagentFrame.countTextQ1 = CraftSim.FRAME:CreateText(" x ???", reagentFrame, reagentFrame.icon.frame, "LEFT",
                "RIGHT", 5, 12, 0.9, nil, { type = "HV", valueH = "LEFT", valueV = "CENTER" })
            reagentFrame.countTextQ2 = CraftSim.FRAME:CreateText(" x ???", reagentFrame, reagentFrame.icon.frame, "LEFT",
                "RIGHT", 5, 0, 0.9, nil, { type = "HV", valueH = "LEFT", valueV = "CENTER" })
            reagentFrame.countTextQ3 = CraftSim.FRAME:CreateText(" x ???", reagentFrame, reagentFrame.icon.frame, "LEFT",
                "RIGHT", 5, -12, 0.9, nil, { type = "HV", valueH = "LEFT", valueV = "CENTER" })
        ...
        frame.content.reagentFrames = {
            ...
            createReagentFrame("TOPLEFT", frame.content.reagentDetailsTitle, "BOTTOMLEFT", baseX + spacingX,
                baseY + spacingY),
            createReagentFrame("TOPLEFT", frame.content.reagentDetailsTitle, "BOTTOMLEFT", baseX + spacingX * 2,
                baseY + spacingY),
        }
...
function CraftSim.CUSTOMER_SERVICE.FRAMES:InitLivePreviewSession(payload)
    ...
    previewFrame.content.recipeDropdown:SetData({
        data = convertToDropdownListData(recipes),
        initialValue = CraftSim
            .LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_LEARNED_RECIPES_INITIAL)
    })
...
function CraftSim.CUSTOMER_SERVICE.FRAMES:UpdateRecipe(payload)
    ...
    if resultData.canUpgradeQuality then
        ...
        previewFrame.content.expectedInspirationPercent:SetText(upgradeChanceText ..
            CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_EXPECTED_INSPIRATION))
    ...
    for i, reagent in pairs(reagents) do
        for _, reagentItem in pairs(reagent.items) do
            local reagentCost = reagentItem.quantity *
                CraftSim.PRICEDATA:GetMinBuyoutByItemID(reagentItem.item:GetItemID(), true)
            print("reagentCost #" .. i .. ": " .. CraftSim.GUTIL:FormatMoney(reagentCost))
            print(reagentItem.quantity ..
                " x " .. CraftSim.PRICEDATA:GetMinBuyoutByItemID(reagentItem.item:GetItemID(), true))
        ...
        for i = 1, 5, 1 do
            ...
            if optionalReagentSlot then
                ...
                if not optionalReagentSlot.locked then
                    local dropdownListData = convertOptionalReagentsToDropdownListData(optionalReagentSlot
                        .possibleReagents)
                    dropdown:SetData({
                        data = dropdownListData,
                        initialValue = CraftSim.LOCAL:GetText(CraftSim.CONST
                            .TEXT.CUSTOMER_SERVICE_REAGENTS_NONE)
                    })
                    ...
                    dropdown:SetData({
                        data = {},
                        initialValue = CraftSim.GUTIL:ColorizeText(
                            CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT.CUSTOMER_SERVICE_REAGENTS_LOCKED),
    ...
    previewFrame.content.craftingCosts:SetText(CraftSim.LOCAL:GetText(CraftSim.CONST.TEXT
        .CUSTOMER_SERVICE_CRAFTING_COSTS) .. "\n" .. CraftSim.GUTIL:FormatMoney(craftingCosts))
