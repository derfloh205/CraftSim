...
function CraftSim.CUSTOMER_SERVICE.OnPreviewRequest(payload)
    ...
    if not CraftSim.UTIL:IsMyVersionHigher(payload.addonVersion) and not versionWarningShowed then
        ...
        CraftSim.GGUI:ShowPopup({
            sizeX = 400,
            sizeY = 250,
            title = "Newer CraftSim Version Detected",
...
function CraftSim.CUSTOMER_SERVICE.OnRecipeUpdateRequest(payload)
    ...
    elseif recipeData.supportsInspiration then
        if withInspiration then
            ...
            upgradeChance = ((inspChance * hsvChance) + ((1 - inspChance) * hsvChance) + (inspChance * (1 - hsvChance))) *
                100
        end
    end
...
function CraftSim.CUSTOMER_SERVICE.OnRecipeUpdateResponse(payload)
    ...
    payload.reagents = CraftSim.GUTIL:Map(payload.reagents,
        function(serializedReagent) return CraftSim.Reagent:Deserialize(serializedReagent) end)
    payload.optionalReagents = CraftSim.GUTIL:Map(payload.optionalReagents,
        function(serializedOptionalReagentSlot)
            return CraftSim.OptionalReagentSlot:Deserialize(
                serializedOptionalReagentSlot)
        end)
    payload.finishingReagents = CraftSim.GUTIL:Map(payload.finishingReagents,
        function(serializedOptionalReagentSlot)
            return CraftSim.OptionalReagentSlot:Deserialize(
                serializedOptionalReagentSlot)
        end)
    ...
    table.foreach(CraftSim.GUTIL:Concat({ payload.optionalReagents, payload.finishingReagents }), function(_, slot)
        itemsToLoad = CraftSim.GUTIL:Concat({ itemsToLoad, CraftSim.GUTIL:Map(slot.possibleReagents,
            function(optionalReagent) return optionalReagent.item end) })
    end)
...
function CraftSim.CUSTOMER_SERVICE:TransformLink(event, message, sender, ...)
    ...
    if linkType == "CraftSimLivePreview" then
        ...
        local link = "|cffe6cc80|HCraftSimLivePreview:" .. data .. "|h[CraftSim Live Preview: " ..
            professionName .. "]|h|r"
...
function CraftSim.CUSTOMER_SERVICE:WhisperRecipeDetails(whisperTarget)
    ...
    table.foreach(requiredReagents, function(_, reagent)
        table.foreach(reagent.items, function(_, reagentItem)
            if reagentItem.quantity > 0 then
                local itemPrice = (CraftSim.PRICEDATA:GetMinBuyoutByItemID(reagentItem.item:GetItemID(), true) or 0) *
                    reagentItem.quantity
                local itemPriceFormatted = CraftSim.GUTIL:GetMoneyValuesFromCopper(
                    CraftSim.GUTIL:Round(itemPrice * 10000) / 10000, true)
                detailedCraftingCostText = detailedCraftingCostText ..
                    reagentItem.item:GetItemLink() ..
                    " x " .. reagentItem.quantity .. " (~" .. itemPriceFormatted .. ")" .. "\n"
                reagentListText = reagentListText ..
                    reagentItem.item:GetItemLink() .. " x " .. reagentItem.quantity .. "\n"
            end
        end)
    end)
    ...
    table.foreach(optionalReagents, function(_, optionalReagent)
        ...
        local itemPriceFormatted = CraftSim.GUTIL:GetMoneyValuesFromCopper(CraftSim.GUTIL:Round(itemPrice * 10000) /
            10000, true)
        detailedCraftingCostText = detailedCraftingCostText ..
            optionalReagent.item:GetItemLink() .. " x 1 (~" .. itemPriceFormatted .. ")" .. "\n"
    ...
    responseText = string.gsub(responseText or "", "%%ic",
        (resultData.canUpgradeInspiration and resultData.expectedItemInspiration:GetItemLink()) or "-")
