...
function CraftSim.RECIPE_INFO.UI:Init()
    ...
    self.module.frame = CraftSim.RECIPE_INFO.frame
end
...
function CraftSim.RECIPE_INFO.UI:BuildDisplayState(recipeData, statWeights)
    ...
    if statWeights and recipeData.supportsMulticraft then
        local formatted = CraftSim.UTIL:FormatMoney(statWeights.multicraftWeight)
        if formatted ~= "" then
            addTextRow("MULTICRAFT_WEIGHT",
                L("MULTICRAFT_LABEL"),
                formatted,
    ...
    if statWeights and recipeData.supportsResourcefulness then
        local formatted = CraftSim.UTIL:FormatMoney(statWeights.resourcefulnessWeight)
        if formatted ~= "" then
            addTextRow("RESOURCEFULNESS_WEIGHT",
    ...
    if statWeights and recipeData.supportsQualities and recipeData.concentrationCost > 0 then
        local formatted = CraftSim.UTIL:FormatMoney(statWeights.concentrationWeight)
        if formatted ~= "" then
            addTextRow("CONCENTRATION_WEIGHT",
    ...
    addIconsRow("RESULT_ICONS", recipeData.resultData.itemsByQuality,
        f.white(f.bb("Icons") .. " for each possible result item quality"))

    if recipeData.specializationData then
        local allocated, maxKP = CraftSim.RECIPE_INFO:GetKnowledgePoints(recipeData)
        addTextRow("KNOWLEDGE_POINTS",
    ...
    if recipeData.supportsMulticraft then
        ...
        addTextRow("AVG_MULTICRAFT_ITEMS",
    ...
    if recipeData.supportsResourcefulness then
        addTextRow("AVG_RESOURCEFULNESS_SAVED",
    ...
    if recipeData.supportsQualities and recipeData.concentrationCost > 0 then
        local _, concProfit = recipeData:GetConcentrationValue()
        addTextRow("CONCENTRATION_PROFIT",
    ...
    return {
        priceOverridesActive = recipeData.priceData:PriceOverridesActive(),
        rows = rows,
    }
end
