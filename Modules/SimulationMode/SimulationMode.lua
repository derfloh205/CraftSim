...
function CraftSim.SIMULATION_MODE:OnInputAllocationChangedOOP(inputBox, userInput)
    local recipeData = CraftSim.SIMULATION_MODE.recipeData
    if not userInput or not recipeData then
        return
    end
...
function CraftSim.SIMULATION_MODE:AllocateAllByQualityOOP(qualityID)
    for _, currentInput in pairs(CraftSim.SIMULATION_MODE.reagentOverwriteFrame.reagentOverwriteInputs) do
...
function CraftSim.SIMULATION_MODE:UpdateProfessionStatModifiersByInputsOOP()
    local recipeData = CraftSim.SIMULATION_MODE.recipeData
...
function CraftSim.SIMULATION_MODE:UpdateRequiredReagentsByInputsOOP()
    local recipeData = CraftSim.SIMULATION_MODE.recipeData
    -- should not happen but nil check anyway
    if not recipeData then
        return
...
function CraftSim.SIMULATION_MODE:UpdateSimulationModeOOP()
